...
walk_knees = np.array(walk_knees)
walk_shoulders = np.array(walk_shoulders)
#These next 2 lines swap the first 2 columns to make opposite diagonal feet match
walk_knees[:, [0, 1]] = walk_knees[:, [1,0]]
walk_shoulders[:, [0, 1]] = walk_shoulders[:, [1,0]]
#walk_shoulders[:, 0] *= -1
#walk_shoulders[:, 3] *= -1
#walk_shoulders[:, 1] -= np.pi/4
#walk_shoulders[:, 2] -= np.pi/4
print(walk_knees)
walk_knees = [[0, 0, 0, 0]]
walk_shoulders = [[0, 0, 0, 0]]

walk_shoulders = [[0, 0, 0, 0], [np.pi/4, -np.pi/4, np.pi/4, -np.pi/4],[None, None, None, None],[None, None, None, None]]
walk_knees = [[0, 0, 0, 0], [np.pi/4, np.pi/4, np.pi/4, np.pi/4],[np.pi/2, np.pi/2, np.pi/2, np.pi/2], [np.pi/4, np.pi/4, np.pi/4, np.pi/4]]
